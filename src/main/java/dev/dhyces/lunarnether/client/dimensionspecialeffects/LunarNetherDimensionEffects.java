...
public class LunarNetherDimensionEffects extends DimensionSpecialEffects {
    ...
    private void renderLunarNetherSkybox(ClientLevel level, Matrix4f modelViewMatrix, Matrix4f projectionMatrix) {
        ...
        Matrix4f overworldMatrix = new Matrix4f(modelViewMatrix).rotateY((float) Math.toRadians(90F)).rotateX(-(float) Math.toRadians(-125.0F));
        Matrix4f overworldGlowMatrix = new Matrix4f(modelViewMatrix).rotateY((float) Math.toRadians(90F)).rotateX(-(float) Math.toRadians(-125.0F));
